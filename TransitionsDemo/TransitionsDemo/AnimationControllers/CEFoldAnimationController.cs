...
namespace TransitionsDemo.AnimationControllers
{
	public class CEFoldAnimationController : CEReversibleAnimationController
	{
		                                        ...
		                                        UIView fromView, UIView toView)
		{
			...
			toView.Center = new PointF (1000f, 0f);
			...
			SizeF size = toView.Frame.Size;

			float foldWidth = size.Width * 0.5f / Folds;
			...
			for (int i = 0; i < Folds; i++) {
				...
				leftFromViewFold.Layer.Position = new PointF (offset, size.Height / 2f);
				...
				rightFromViewFold.Layer.Position = new PointF (offset + foldWidth * 2f, size.Height / 2f);
				...
				leftToViewFold.Layer.Position = new PointF (Reverse ? size.Width : 0f, size.Height / 2f);
				...
				rightToViewFold.Layer.Position = new PointF (Reverse ? size.Width : 0f, size.Height / 2f);
			...
			fromView.Center = new PointF (1000f, 0f);
			...
			NSAction animation = () => {
				for (int i = 0; i < Folds; i++) {
					...
					leftFromView.Layer.Position = new PointF (Reverse ? 0f : size.Width, size.Height / 2f);
					...
					rightFromView.Layer.Position = new PointF (Reverse ? 0f : size.Width, size.Height / 2f);
					...
					leftToView.Layer.Position = new PointF (offset, size.Height / 2f);
					leftToView.Layer.Transform = CATransform3D.Identity;
					leftToView.Subviews [1].Alpha = 0f;

					UIView rightToView = toViewFolds [i * 2 + 1];
					rightToView.Layer.Position = new PointF (offset + foldWidth * 2f, size.Height / 2f);
		...
		private UIView CreateSnapshot (UIView view, bool afterUpdates, float offset, bool left)
		{
			...
			if (!afterUpdates) {
				...
				var snapshotRegion = new RectangleF (offset, 0f, foldWidth, size.Height);
				...
				snapshotView = new UIView (new RectangleF (0f, 0f, foldWidth, size.Height));
			...
			snapshotWithShadowView.Layer.AnchorPoint = new PointF (left ? 0f : 1f, 0.5f);
		...
		private UIView AddShadowToView (UIView view, bool left)
		{
			...
			gradient.StartPoint = new PointF (Reverse ? 0f : 1f, Reverse ? 0.2f : 0f);
			gradient.EndPoint = new PointF (Reverse ? 1f : 0f, Reverse ? 0f : 1f);
