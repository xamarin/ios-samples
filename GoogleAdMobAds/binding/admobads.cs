...
{	
	...
	///<summary>
	///<remarks>
	///<para>Google AdMob Ads banners allow the application user to click-through to a full-screen advertising experience.
	///The user may exit that full-screen experience either by returning to the application or by activating other applications
	///such as iTunes.</para>
	///<para>There may be a delay of up to 2 minutes the first time AdMob sees the application's publisher ID (see 
	///<see cref="T:GoogleAdMobAds.GADBannerView.AdUnitId"/>) in any 24-hour period. Application developers should </para>
	///</remarks>
	...
	interface GADBannerView
	{
		...
		///<summary>The <see cref="T:MonoTouch.UIKit.UIViewController"/> to #tk check -> # restore after the ad has been visited</summary>
		...
		///<summary>#tk check# Requests an ad to be displayed in this <see cref="T:GoogleAdMobAds.GADBannerView"/>.</summary>
		...
		///</list>
		///</para>
		///<para>#check What about Smart Banners? Can't find any relevant constants in the binding code.</para>
		///<para>If the <see cref="T:GoogleAdMobAds.GADBannerView"/> is smaller than the smallest size (320x50), no ads will be served to it.</para>
		///<para>Banner will auto-refresh every 30-120 seconds, if the application developer's AdMob account on the server specifies auto-refresh.
		///Otherwise, banners can be programmatically refreshed by
		///calling <see cref="M:GoogleAdMobAds.GADBannerView.LoadRequest"/>.</para>
		/// <para>Ad targeting can be improved by specifying demographic data in the <paramref name="request"/>.</para>
		///</remarks>
		...
		///<param name="frame">The <see cref="T:System.Drawing.RectangleF"/> specifying the frame for the <see cref="T:GoogleAdMobAds.GADBannerView"/>
	...
	interface GADBannerViewDelegate
	{
		...
		///<remarks>This method is an appropriate place for the application developer to add the <paramref name="view"/> to the view hierarchy if it has been hidden.</remarks>
		///<param name="view">The <see cref="T:GoogleAdMobAds.GADBannerView"/> associated with the event.</para>
		...
		///<param name="error">A <see cref="T:GoogleAdMobAds.GADRequestError"/> with the specifics of the failure</param>
		///<param name="view">The <see cref="T:GoogleAdMobAds.GADBannerView"/> associated with the event.</para>
		...
		///<param name="view">The <see cref="T:GoogleAdMobAds.GADBannerView"/> associated with the event.</para>
		///<remarks>The application developer should take the same steps with the application that they would take if it were being sent to the background.
		///This might include pausing animations and timers and saving application state.</remarks>
		...
		///<param name="view">The <see cref="T:GoogleAdMobAds.GADBannerView"/> associated with the event.</para>
		///<remarks>The application developer should prepare for their application to be brought to the foreground, reversing the steps taken 
		///in <see cref="T:GoogleAdMobAds.GADBannerViewDelegate.AdViewWillPresentScreen"/>.</remarks>
	...
	///method to be called.  
	///</para>
	///<para>All publishers can run interstitial house ads, but paid interstitial ads are only offered to some publishers. Google's documntation states "if you become eligible, Google will be sure to contact you."</para>
	///</remarks>
	...
	interface GADInterstitial
	{
		...
		///<summary>The <see cref="T:GoogleAdMobAds.GADBInterstitialDelegate"/> delegate with which to handle events relating to this <see cref="T:GoogleAdMobAds.GADInterstitial"/>.</summary>
		...
		///</para>
		///<para>The application developer must not attempt to display the content of an interstitial until 
		...
		///<summary>Used to create a "splash interstitial." The <paramref name="image"/> will display until the load request succeeds or fails.</summary>
		///<param name="request">The <see cref="T:GoogleAdMobAds.GADRequest"/> for an ad</param>
		///<param name="image">The initial image (such as a splash image) to display prior to the interstitial load completing</param>
		///<param name="window">The <see cref="T:MonoTouch.UIKit.UIWindow"/> in which the interstitial will appear #tk check</param>
		///<remarks>
		/// <para>A "splash interstitial" appears at app launch. Typically, one calls this method a notification-handler registered with the 
		...
		/// screen. It remains on-screen until the load request succeeds or fails.</para>
	...
	interface GADRequest
	{
		...
		///<remarks>The identifier of a device is the value of its <see cref="P:MonoTouch.UIKit.UIDevice.UniqueIdentifier"/> property.</remarks>
