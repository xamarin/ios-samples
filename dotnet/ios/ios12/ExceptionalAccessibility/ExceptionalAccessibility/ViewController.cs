...
{
        const string CellIdentifier = "dog collection view cell";

        readonly List<Dog> dogs = Dog.All;

        Dog? currentlyFocusedDog;

        protected ViewController (IntPtr handle) : base (handle) { }

        protected Dog? CurrentlyFocusedDog
        {
                get => currentlyFocusedDog;

                // Every time we update our Dog object, we need to relay that change to all the views that care.
                set
                {
                        if (currentlyFocusedDog != value)
                        {
                                currentlyFocusedDog = value;
                                if (dogStatsView is not null)
                                {
        ...
        public override void ViewDidLoad ()
        {
                ...
                shelterInfoView.AccessibilityCustomActions = new UIAccessibilityCustomAction []
                {
        ...
        /// </summary>
        bool ActivateCallButton (UIAccessibilityCustomAction arg)
        {
                return true;
        }
        ...
        /// </summary>
        bool ActivateLocationButton (UIAccessibilityCustomAction arg)
        {
                return true;
        }
        ...
        partial void galleryButtonPressed (UIButton sender)
        {
                ...
                if (dogModalViewController is null)
                {
                ...
                if (dogModalViewController.View is DogModalView dogModalView)
                {
                        ...
                        if (currentlyFocusedDog?.Images?.Count >= 2)
                        {
        ...
        public UICollectionViewCell GetCell (UICollectionView collectionView, NSIndexPath indexPath)
        {
                ...
                {
                        var dog = dogs [(int)indexPath.Item];
        ...
        {
                if (dogCollectionView?.CollectionViewLayout is UICollectionViewFlowLayout layout)
                {
        ...
        {
                if (dogCollectionView.CollectionViewLayout is UICollectionViewFlowLayout flowLayout)
                {
                        var itemWidth = flowLayout.ItemSize.Width;
                        var offset = dogCollectionView.ContentOffset.X / itemWidth;
                        var index = (int)Math.Round (offset);

                        if (index >= 0 && index < dogs.Count)
                        {
                                var focusedDog = dogs [index];
                                CurrentlyFocusedDog = focusedDog;

                                if (focusedDog?.Images?.Count > 1)
                                {
                                        if (galleryButton.Alpha == 0f)
                                        {
                                                UIView.Animate (0.25d, () => galleryButton.Alpha = 1f);
                                        }
                                }
                                else if (galleryButton.Alpha == 1f)
                                {
                                        UIView.Animate (0.25d, () => galleryButton.Alpha = 0f);
                                }
