...
internal class BoundingBox : SCNNode
{
	...
	double maxDistanceToFocusPoint = 0.05;


	NVector3 extent = new NVector3 (0.1F, 0.1F, 0.1F);
	...
	NVector3 Max (NVector3 value, double min)
	{
	...
	struct SideDrag
	{
		...
		internal SideDrag (BoundingBoxSide side, NMatrix4 transform, SCNVector3 worldPosition, NVector3 extent) : this ()
		{
			...
			BeginWorldPos = worldPosition;
			BeginExtent = extent;
		}
	}

	SideDrag? currentSideDrag;
	PlaneDrag? currentSidePlaneDrag;
	PlaneDrag? currentGroundPlaneDrag;

	Wireframe? wireframe;

	SCNNode sidesNode = new SCNNode ();
	Dictionary<BoundingBoxSide.PositionName, BoundingBoxSide> sides = new Dictionary<BoundingBoxSide.PositionName, BoundingBoxSide> ();

	UIColor color = Utilities.AppYellow;

	List<(Ray, NVector3)> cameraRaysAndHitLocations = new List<(Ray, NVector3)> ();

	int frameCounter = 0;

	internal int ProgressPercentage { get; private set; }

	bool updatingCaptureProcess = false;

	ARSCNView sceneView;

	bool snappedToHorizontalPlane = false;

	NSObject notificationObserverHandle;
	...
	BoundingBoxSide? AncestralBoundingBox (SCNNode node)
	{
		...
		if (node.ParentNode is BoundingBoxSide bbside)
		{
			return bbside;
		}
	...
	{
		var camera = sceneView.PointOfView;
		if (camera is null || Contains (camera.WorldPosition))
		{
			return;
		}
	...
	(Tile, NVector3)? TileHitBy (Ray ray)
	{
		...
		foreach (var result in hitResults)
		{
			if (result.Node is Tile tile)
			{
				var side = tile.ParentNode as BoundingBoxSide;
				if (side is null || side.IsBusyUpdatingTiles)
				{
					continue;
				}
	...
	bool HitLocationDifferentFromPreviousRayHitTests (NVector3 location)
	{
	...
	bool Contains (SCNVector3 pointInWorld)
	{
		var localMin = Extent.Times (-0.5F);
		var localMax = Extent.Times (0.5F);
