...
{
	double lineThickness = 0.002;
	List<SCNNode> lineNodes = new List<SCNNode> ();
	UIColor color = Utilities.AppYellow;
	NSTimer? flashTimer;
	const double flashDuration = 0.1;
	...
	SCNGeometry Cylinder (float height, float? width = null, UIColor colorArg = null)
	{
		var radius = width is null ? (float)lineThickness / 2 : width.Value / 2;
	...
	void Flash (NSNotification notification)
	{
		Highlighted = true;
