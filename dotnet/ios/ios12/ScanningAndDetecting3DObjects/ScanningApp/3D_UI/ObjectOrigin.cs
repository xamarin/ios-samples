...
internal class ObjectOrigin : SCNNode
{
	...
	const float axisThickness = 6.0F; // Axis thickness in percentage of length. 

	const float axisSizeToObjectSizeRatio = 0.25F;
	const float minAxisSize = 0.05F;
	const float maxAxisSize = 0.2F;

	ObjectOriginAxis xAxis;
	ObjectOriginAxis yAxis;
	ObjectOriginAxis zAxis;

	SCNNode? customModel;

	PlaneDrag? currentAxisDrag;
	PlaneDrag? currentPlaneDrag;
	...
	bool snappedToSide = false;
	bool snappedToBottomCenter = false;
	bool snappedTo90DegreeRotation = false;
	float totalRotationSinceLastSnap = 0;

	NSObject [] notificationObserverHandles;

	bool DisplayingCustom3DModel { get => customModel is not null; }
	...
	void AdjustToExtent (NVector3? extent)
	{
		if (extent is null)
		{
		...
		var axesScale = new [] { extent!.Value.X, extent!.Value.Y, extent!.Value.Z }.Min () * axisSizeToObjectSizeRatio;
		...
		if (customModel is not null)
		{
			...
			var originScale = (float)(new [] { extent!.Value.X, extent!.Value.Y, extent!.Value.Z }.Min () / modelExtent);
