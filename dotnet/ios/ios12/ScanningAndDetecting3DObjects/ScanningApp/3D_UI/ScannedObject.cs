...
internal class ScannedObject : SCNNode
{
	...
	void CreateBoundingBoxFromGhost ()
	{
		if (ghostBoundingBox is null)
		{
			return;
		}
		boundingBox = ghostBoundingBox;
	...
	void CreateBoundingBox (CGPoint screenPos)
	{
		// Perform a hit test against the feature point cloud. 
		var result = sceneView.SmartHitTest (screenPos);
		if (result is null)
	...
	void UpdateOrCreateGhostBoundingBox ()
	{
		// Perform a hit test against the feature point cloud. 
		...
		if (result is null)
		{
			...
			if (ghostBoundingBox is not null && !busyCreatingReferenceObject)
			{
		...
		if (ghostBoundingBox is not null)
		{
	...
	void MoveOriginToBottomOfBoundingBox ()
	{
		...
		if (boundingBox is null || Origin is null || Origin.PositionHasBeenAdjustedByUser)
		{
			return;
		}
