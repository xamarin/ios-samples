...
internal partial class ViewController : UIViewController, IUIGestureRecognizerDelegate
{
	...
	ViewControllerApplicationState? state;
	...
	internal void EnterStateDefineBoundingBox ()
	{
		...
		navigationBarController?.ShowBackButton (true);
		nextButton.Enabled = scan is not null ? scan.BoundingBoxExists : false;
		loadModelButton.Hidden = true;
		flashlightButton.Hidden = true;
	...
	internal void EnterStateScanReady ()
	{
		...
		loadModelButton.Hidden = true;
		flashlightButton.Hidden = true;
		if (scan is not null && scan.GhostBoundingBoxExists)
		{
	...
	internal void RestartButtonTapped (Object sender, EventArgs args)
	{
		...
		if (scan is not null && scan.BoundingBoxExists)
		{
			startOverAlert ();
		}
		else
	...
	partial void LoadModelButtonTouched (NSObject sender)
	{
		...
		if (documentPicker.PopoverPresentationController is not null)
		{
	...
	{
		if (scan is null || !scan.BoundingBoxExists)
		{
	...
	{
		if (testRun is null || testRun.ReferenceObject is null || testRun.ReferenceObject.Name is null)
		{
	...
	{
		if (limitedTrackingTimer is not null)
		{
	...
	{
		if (maxScanTimeTimer is not null)
		{
	...
	T? TryGet<T> (NSDictionary? dict, NSObject key) where T : NSObject
	{
	...
	void DisplayWarningIfInLowPowerMode (NSNotification? notification)
	{
	...
	void ObjectOriginPositionChanged (NSNotification notification)
	{
	...
	void BoundingBoxPositionChanged (NSNotification notification)
	{

		var boundingBox = notification.Object as BoundingBox;
		var cameraPos = sceneView.PointOfView?.WorldPosition;
		if (boundingBox is null || !cameraPos.HasValue)
		{
			return;
		}
	...
	void ScanPercentageChanged (NSNotification notification)
	{
	...
	internal void BackFromBackground ()
	{
		if (state?.CurrentState == AppState.Scanning)
		{
			...
			var buttonTitle = "Restart scan";
			ShowAlert (title, message, buttonTitle, true, (_) => { if (State is not null) State.CurrentState = AppState.NotReady; });
		}
	}
