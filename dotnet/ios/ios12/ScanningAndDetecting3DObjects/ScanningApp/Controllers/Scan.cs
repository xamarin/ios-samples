...
Do you want to go back and adjust the bounding box of the scanned object?""";
			...
			switch (value)
			{
				...
				case ScanState.AdjustingOrigin:
					...
					if (state == ScanState.Scanning)
					{
						...
						if (boundingBox is not null && boundingBox.ProgressPercentage < 100)
						{
	...
	{
		if (!(notification?.UserInfo? [ViewControllerApplicationState.AppStateUserInfoKey] is SimpleBox<AppState> newAppState))
			return;
	...
	internal void UpdateOnEveryFrame (ARFrame frame)
	{
		...
		{
			if (frame.RawFeaturePoints is not null)
			{
		...
		{
			if (frame.LightEstimate is not null)
			{
			...
			if (scannedObject.EitherBoundingBox is not null)
			{
	...
	internal bool BoundingBoxExists => scannedObject.BoundingBox is not null;

	internal bool GhostBoundingBoxExists => scannedObject.GhostBoundingBox is not null;

	bool IsReasonablySized ()
	{
		if (scannedObject.BoundingBox is null)
		{
			return false;
		}
	...
	{
		if (scannedObject.BoundingBox is null || scannedObject.Origin is null)
		{
	...
	{
		if (sceneView.Session.CurrentFrame is null)
		{
