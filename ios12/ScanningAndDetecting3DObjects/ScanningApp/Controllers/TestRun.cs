...
namespace ScanningAndDetecting3DObjects {
	...
	internal class TestRun : NSObject, IDisposable {
		private ViewControllerSessionInfo sessionInfo;
		private ARSCNView sceneView;
		private int detections = 0;
		private double lastDetectionDelayInSeconds = 0.0;
		private double averageDetectionDelayInSeconds = 0.0;
		private DateTime lastDetectionStartTime;

		private NSTimer noDetectionTimer;
		...
		internal void SuccessfulDetection (ARObjectAnchor objectAnchor)
		{
			...
			if (sceneView.Session.CurrentFrame?.RawFeaturePoints != null) {
		...
		protected override void Dispose (bool disposing)
		{
			if (disposing) {
				...
				if (sceneView.Session.Configuration as ARWorldTrackingConfiguration != null) {
